...
def donchian_ensemble_pyramid_seasonal(close, pairs=None, threshold=0.5,
                                       entry=20, exit_period=10,
                                       target_vol=0.15, vol_window=30,
                                       max_pos=2.0, pyramid_atr=1.5,
                                       atr_period=14):
    ...
    for i in range(len(close)):
        ...
        if in_position:
            ...
            month_ok = close.index[i].month in (8, 9, 10, 11)
            if price > high_n.iloc[i] and macro_bullish.iloc[i] and month_ok:
                in_position = True
                pyramid_done = False
                entry_price = price
                raw_position.iloc[i] = 0.5
...
def donchian_seasonal_voltarget(close, entry=20, exit_period=10,
    ...
    Same signal as donchian_seasonal (calendar-filtered breakout),
    but position size scaled by target_vol / realised_vol.
    Reduces exposure in high-volatility periods (gas spikes).
    ...
    Args:
        ...
        buy_months: Months when entries are allowed. Defaults to
            (8, 9, 10, 11).
        target_vol: Target annualised volatility. Defaults to 0.15.
        vol_window: Rolling window for vol estimate. Defaults to 30.
        max_pos: Leverage cap. Defaults to 2.0.

    Returns:
        position: Fractional Series (vol-scaled), same index as close.
    """
